...
function sortByTimeDesc(
  ...
  return [...items].sort((a, b) => {
    const ta =
      Date.parse(
        (field === "updatedAt" ? (a.updatedAt ?? a.createdAt) : a.createdAt) ??
          "",
      ) || 0;
    const tb =
      Date.parse(
        (field === "updatedAt" ? (b.updatedAt ?? b.createdAt) : b.createdAt) ??
          "",
      ) || 0;
    return tb - ta;
  });
...
export function HomeMarketplaceRails() {
  ...
  const featured = useMemo(
    () => sortByTimeDesc(raw, "updatedAt").slice(0, ROW_SIZE),
    [raw],
  );
  const latest = useMemo(
    () => sortByTimeDesc(raw, "createdAt").slice(0, ROW_SIZE),
    [raw],
  );
  ...
  if (!universityId) {
    return (
      <section
        id="home-used-marketplace"
        aria-labelledby="home-used-marketplace-heading"
      >
        <SectionWrapper
          spacing="none"
          background="transparent"
          className="my-0 bg-gray-50/90 py-8  md:py-16"
        >
          <ContentWrapper
          ...
          >
            <div className="flex items-start gap-4 rounded-2xl border border-dashed border-gray-200 bg-white p-8">
              ...
              <div>
                ...
                >
                  {tt(
                    "homeRails.marketplaceTitle",
                    "Used & second-hand marketplace",
                  )}
                </h2>
  ...
  return (
    ...
    >
      <SectionWrapper
        spacing="none"
        background="transparent"
        className="my-0 bg-gray-50/90 py-12 md:py-16"
      >
        <ContentWrapper
          maxWidth="max-w-7xl mx-auto"
          padding="none"
