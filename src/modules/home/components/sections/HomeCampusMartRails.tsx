  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!cancelled) {
          setError(e instanceof Error ? e.message : "Failed to load Campus Mart.");
          setShops([]);
          setProducts([]);
        }
      } finally {
        ...
        <SectionWrapper spacing="none" background="transparent" className="my-0 border-t border-gray-100 bg-white py-12 md:py-16">
          <ContentWrapper maxWidth="max-w-7xl mx-auto" padding="none" className="px-4 md:px-8">
            <div className="flex items-start gap-4 rounded-2xl border border-dashed border-gray-200 bg-gray-50/80 p-8">
              ...
              <div>
                <h2 id="home-campus-mart-heading" className="text-lg font-bold text-gray-900 md:text-xl">
                  {tt("homeRails.campusMartTitle", "Campus Mart — shops & products")}
                </h2>
        ...
        <ContentWrapper maxWidth="max-w-7xl mx-auto" padding="none" className="px-4 md:px-8">
          <div className="flex flex-col gap-2 border-b border-gray-200/80 pb-8 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <h2 id="home-campus-mart-heading" className="mt-2 text-2xl font-bold tracking-tight text-gray-900 md:text-3xl">
                {tt("homeRails.campusMartHeadline", "Official shops & retail products")}
              </h2>
          ...
          <div className="mt-10 space-y-12">
            <SectionWrapper spacing="sm" background="transparent" className="my-0">
              <SectionHeader
                ...
                subtitle={tt("homeRails.campusMartStoresSub", "Retail outlets on Campus Mart.")}
              ...
              {loading && shops.length === 0 ? (
                ...
                <p className="mt-4 rounded-xl border border-dashed border-gray-200 bg-gray-50 px-4 py-10 text-center text-sm text-gray-500">
                  {tt("homeRails.campusMartEmptyShops", "No stores to show yet for this campus.")}
                </p>
              ) : (
                <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <SectionWrapper spacing="sm" background="transparent" className="my-0">
              <SectionHeader
                title={tt("homeRails.campusMartFeaturedTitle", "Featured products")}
                subtitle={tt("homeRails.campusMartFeaturedSub", "Highlighted listings from campus shops.")}
              ...
              {loading && products.length === 0 ? (
                ...
                <p className="mt-4 rounded-xl border border-dashed border-gray-200 bg-gray-50 px-4 py-10 text-center text-sm text-gray-500">
                  {tt("homeRails.campusMartEmptyProducts", "No featured products right now.")}
                </p>
              ) : (
                <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
