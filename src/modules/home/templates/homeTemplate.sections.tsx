...
function FeatureCard({
  ...
  return (
    <div className="card p-6 group hover:-translate-y-1.5 transition-all duration-300" style={{ animationDelay: `${index * 80}ms` }}>
...
export function HeroSection({ locale }: { locale: string }) {
  ...
  const selectedRole = HERO_ROLES.find((role) => role.id === activeRole) || HERO_ROLES[0];

  useEffect(() => {
    const timer = setInterval(() => setActiveModule((prev) => (prev + 1) % MODULES.length), 2200);
    return () => clearInterval(timer);
  }, []);

  useEffect(() => {
    const timer = setInterval(() => setActiveSignal((prev) => (prev + 1) % HERO_LIVE_SIGNALS.length), 3200);
    return () => clearInterval(timer);
  }, []);
          ...
          <div className="space-y-8 animate-fade-up">
            ...
            <div className="p-4 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm space-y-3">
              ...
              <div className="flex flex-wrap gap-3">
                <Link href={`/${locale}${selectedRole.primary.href}`} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold text-white transition-all duration-200 hover:-translate-y-0.5" style={{ background: "linear-gradient(135deg, #00A651, #00c460)" }}>
                  <selectedRole.primary.icon className="w-4 h-4" />
                  {selectedRole.primary.label}
                </Link>
                <Link href={`/${locale}${selectedRole.secondary.href}`} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold text-white border border-white/15 hover:bg-white/10 transition-all duration-200">
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/8 border border-white/10 backdrop-blur-sm" style={{ background: "rgba(255,255,255,0.06)" }}>
                <div className="w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-300" style={{ background: MODULES[activeModule].bg }}>
                ...
                <span className="text-sm font-semibold text-white/90 min-w-[140px] transition-all duration-300">{MODULES[activeModule].label}</span>
              </div>
              <span className="text-white/40 text-sm">+ integrated live operations</span>
            </div>

            <div className="flex flex-wrap gap-3">
              <Link href={`/${locale}/login`} id="hero-get-started-btn" className="inline-flex items-center gap-2 px-7 py-3.5 rounded-xl font-semibold text-base text-white shadow-lg transition-all duration-200 hover:shadow-glow-lg hover:-translate-y-0.5 active:scale-[0.97]" style={{ background: "linear-gradient(135deg, #00A651, #00c460)" }}>
                Get Started Free
                <ArrowRight className="w-4 h-4" />
              </Link>
              <Link href={`/${locale}/features`} id="hero-explore-features-btn" className="inline-flex items-center gap-2 px-7 py-3.5 rounded-xl font-semibold text-base text-white border border-white/15 hover:bg-white/10 transition-all duration-200 active:scale-[0.97]">
        ...
        <div className="mt-12 pt-8 border-t border-white/8">
            ...
            {STATS.map((stat) => (
              <div key={stat.label} className="text-center lg:text-left">
                <div className="inline-flex items-center gap-2 mb-1.5">
                  ...
                  <span className="font-display font-extrabold text-3xl lg:text-4xl text-white">{stat.value}</span>
                </div>
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3">
            {[
              ...
              <span key={item} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-white/10 text-xs text-white/60 bg-white/[0.03]">
  ...
  return (
    <>
      {FEATURES.map((group, index) => (
        <Section key={group.tag} className={`py-20 ${index % 2 === 0 ? "bg-white" : "bg-neutral-50"}`}>
    ...
    <Section id="how-it-works" className="py-20 bg-brand-navy-700 relative overflow-hidden">
      ...
      <div className="cs-container relative z-10">
        ...
        <div className="text-center mt-14">
          <Link href="/login" id="hiw-cta-btn" className="inline-flex items-center gap-2 px-8 py-4 rounded-xl font-semibold text-base text-white shadow-lg transition-all duration-200 hover:-translate-y-0.5 active:scale-[0.97]" style={{ background: "linear-gradient(135deg, #00A651, #00c460)" }}>
    ...
    <Section className="py-20 bg-white">
      <div className="cs-container">
        <SectionHeader
          ...
          title={
            <>
              What Our <span className="gradient-text-green">Community</span> Says
            </>
          }
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {TESTIMONIALS.map((testimonial, index) => (
            <div key={testimonial.name} className="card p-7 hover:-translate-y-1 transition-all duration-300" style={{ animationDelay: `${index * 100}ms` }}>
              <div className="flex items-start justify-between mb-5">
                <Stars />
                <span className="badge-green text-[10px]">{testimonial.module}</span>
              </div>
              <p className="text-neutral-600 text-sm leading-relaxed mb-6 italic">&ldquo;{testimonial.text}&rdquo;</p>
              <div className="flex items-center gap-3">
          ...
          <div className="p-8 lg:p-12 grid lg:grid-cols-2 gap-8 items-center">
            ...
            <div className="space-y-3">
              {BLOOD_REQUESTS.map((request, index) => (
                <div key={index} className="flex items-center gap-4 p-4 bg-white rounded-2xl border border-neutral-100 shadow-xs hover:shadow-sm transition-all duration-200">
                  <div className="w-12 h-12 rounded-xl bg-red-600 flex items-center justify-center flex-shrink-0 shadow-sm">
                    <span className="text-white font-bold text-sm">{request.group}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <p className="text-sm font-semibold text-neutral-800 truncate">{request.location}</p>
                      <span
...
export function UniversitiesSection() {
  return (
    <Section className="py-14 bg-white border-y border-neutral-100">
      <div className="cs-container">
        <p className="text-center text-xs font-semibold text-neutral-400 uppercase tracking-widest mb-8">Serving Students at</p>
        <div className="flex flex-wrap justify-center gap-3">
          {UNIVERSITIES.map((university) => (
            <div key={university} className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-neutral-50 border border-neutral-200 hover:border-brand-green-DEFAULT hover:bg-brand-green-50 transition-all duration-200 cursor-default group">
              <Building2 className="w-3.5 h-3.5 text-neutral-400 group-hover:text-brand-green-DEFAULT transition-colors" />
              <span className="text-sm font-medium text-neutral-600 group-hover:text-brand-green-700 transition-colors">{university}</span>
            </div>
          ))}
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-start">
          <div>
            ...
            <div className="mt-8">
              ...
              <a href="mailto:campussheba24@gmail.com" id="faq-contact-link" className="inline-flex items-center gap-2 text-sm font-semibold text-brand-green-DEFAULT hover:underline">
          ...
          <div className="space-y-3">
            {FAQS.map((faq, index) => (
              <div key={index} id={`faq-item-${index}`} className="bg-white rounded-2xl border border-neutral-100 overflow-hidden hover:border-neutral-200 transition-colors duration-150">
                <button onClick={() => setOpen(open === index ? null : index)} className="w-full flex items-center justify-between gap-4 px-6 py-4 text-left" aria-expanded={open === index}>
                  <span className="text-sm font-semibold text-neutral-800">{faq.q}</span>
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-200 ${open === index ? "bg-brand-green-DEFAULT rotate-45" : "bg-neutral-100"}`}>
                    <span className={`text-lg leading-none font-light ${open === index ? "text-white" : "text-neutral-500"}`}>+</span>
                  </div>
                </button>
                {open === index && (
...
export function CTASection({ locale }: { locale: string }) {
  return (
    <Section className="py-20 bg-white">
      <div className="cs-container">
        <div className="relative rounded-3xl overflow-hidden text-center py-16 px-8" style={{ background: "linear-gradient(135deg, #0D1B2A 0%, #1a3055 60%, #0D1B2A 100%)" }}>
          ...
          <div className="relative z-10">
            ...
            <div className="flex flex-wrap gap-4 justify-center">
              <Link href={`/${locale}/login`} id="cta-main-btn" className="inline-flex items-center gap-2 px-9 py-4 rounded-xl font-bold text-base text-white shadow-lg transition-all duration-200 hover:-translate-y-0.5 hover:shadow-glow-lg" style={{ background: "linear-gradient(135deg, #00A651, #00c460)" }}>
                Get Started - It&apos;s Free
                <ArrowRight className="w-5 h-5" />
              </Link>
              <Link href={`/${locale}/about`} id="cta-learn-btn" className="inline-flex items-center gap-2 px-9 py-4 rounded-xl font-bold text-base text-white border border-white/15 hover:bg-white/10 transition-all duration-200">
                Learn More
              </Link>
            </div>

            <div className="mt-10 flex flex-wrap items-center justify-center gap-6">
              {["University Verified", "100% Secure", "Free to Join", "3+ Campuses"].map((signal) => (
                <div key={signal} className="flex items-center gap-2 text-white/50 text-xs font-medium">
                  <CheckCircle className="w-3.5 h-3.5 text-brand-green-DEFAULT" />
                  {signal}
                </div>
              ))}
...
export function HomeModulesOverlay({ locale }: { locale: string }) {
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 xl:grid-cols-8">
      ...
      >
        <div className="w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-200 group-hover:scale-110 group-hover:shadow-md" style={{ background: "#F0FFF7" }}>
          <LayoutGrid className="w-7 h-7" style={{ color: "#00A651" }} strokeWidth={1.8} />
        </div>
        <div className="text-center">
          <p className="text-sm font-semibold leading-tight" style={{ color: "#00A651" }}>
            Explore All
          </p>
        </div>
      </Link>
