...
export default function CampusLocationDetail() {
  ...
  const uniName =
    loc && typeof loc.university === "object" && loc.university?.name ? loc.university.name : null;
          ...
          <div className="mt-8 space-y-6">
            <div className="rounded-2xl border border-gray-100 bg-gradient-to-br from-white to-emerald-50/40 p-6 shadow-sm ring-1 ring-black/[0.04]">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {loc.isPopular ? (
                  <span className="rounded-lg bg-amber-100 px-2 py-0.5 text-xs font-bold text-amber-900">Popular</span>
                ) : null}
              </div>
              {uniName ? <p className="mt-1 text-sm text-gray-500">{uniName}</p> : null}
              {loc.description ? (
                <p className="mt-4 text-sm leading-relaxed text-gray-700">{loc.description}</p>
              ) : null}
            </div>

            {mapPoints.length > 0 ? (
              <div className="overflow-hidden rounded-2xl border border-gray-200 shadow-md">
                <CampusMapLeaflet locations={mapPoints} selectedId={loc._id} onSelect={onSelect} />
              </div>
            ) : (
