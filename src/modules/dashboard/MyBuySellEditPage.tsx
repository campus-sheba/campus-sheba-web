...
import {
  deleteBuySellListingAction,
  fetchBuySellCategories,
  updateBuySellListingAction,
...
function addressIdFromListing(listing: BuySellListing): string {
...
function photosToMeta(listing: BuySellListing): UploadedMediaMeta[] {
...
export default function MyBuySellEditPage({ listing }: Props) {
  ...
  const [categoryId, setCategoryId] = useState(() => categoryIdFromListing(listing));
  const [condition, setCondition] = useState(() => listing.condition ?? CONDITIONS[2]);
  ...
  const [addressId, setAddressId] = useState(() => addressIdFromListing(listing));

  const [photos, setPhotos] = useState<UploadedMediaMeta[]>(() => photosToMeta(listing));
  ...
  useEffect(() => {
    void (async () => {
      ...
      try {
        ...
        setAddressId((prev) => {
          if (prev) return prev;
          return pickDefaultDeliveryAddressId(list) || "";
        });
      } catch {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    const res = await updateBuySellListingAction(listingId, {
      title: title.trim(),
    ...
    router.push(`/buy-sell/${listingId}`);
  }

  async function onConfirmDelete() {
    setDeleting(true);
    setError(null);
    const res = await deleteBuySellListingAction(listingId);
    setDeleting(false);
    if (!res.success) {
      setError(res.message);
      return;
  ...
  return (
    <div className="space-y-5">
      <div>
        <Link href="/my-buy-sell" className="text-sm font-medium text-gray-500 transition hover:text-gray-900">
          ← My listings
        </Link>
        <div className="mt-2 flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-xl font-bold tracking-tight text-gray-900">Edit listing</h1>
            <p className="mt-1 text-sm text-gray-500">Update details or remove this listing.</p>
          </div>
          {listing.status ? (
        ...
        >
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            <div className="space-y-4">
              <div>
                <label className={labelClass} htmlFor="bse-title">
                  Title
                </label>
                <input
                  id="bse-title"
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <label className={labelClass} htmlFor="bse-brand">
                    Brand
                  </label>
                  <input
                    id="bse-brand"
                ...
                <div>
                  <label className={labelClass} htmlFor="bse-model">
                    Model
                  </label>
                  <input
                    id="bse-model"
              ...
              <div>
                <label className={labelClass} htmlFor="bse-category">
                  Category
                </label>
                <select
                  id="bse-category"
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <label className={labelClass} htmlFor="bse-condition">
                    Condition
                  </label>
                  <select
                    id="bse-condition"
                ...
                <div>
                  <label className={labelClass} htmlFor="bse-qty">
                    Quantity
                  </label>
                  <input
                    id="bse-qty"
              ...
              <div>
                <label className={labelClass} htmlFor="bse-price">
                  Price (৳)
                </label>
                <input
                  id="bse-price"
            ...
            <div className="space-y-4">
              <div>
                <label className={labelClass} htmlFor="bse-address">
              ...
              <div>
                <label className={labelClass} htmlFor="bse-desc">
                  Description
                </label>
                <textarea
                  id="bse-desc"
              ...
              <div className="border-t border-gray-100 pt-4">
                <p className="text-xs font-semibold uppercase tracking-wide text-gray-400">Buyer contact</p>
                <div className="mt-3 space-y-3">
                  <div>
                    <label className={labelClass} htmlFor="bse-cname">
                      Name
                    </label>
                    <input
                      id="bse-cname"
                  ...
                  <div>
                    <label className={labelClass} htmlFor="bse-cphone">
                      Phone
                    </label>
                    <input
                      id="bse-cphone"
                  ...
                  <div>
                    <label className={labelClass} htmlFor="bse-cemail">
                      Email
                    </label>
                    <input
                      id="bse-cemail"
      ...
      {deleteOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-xl border border-gray-200 bg-white p-5 shadow-lg">
            <h2 className="text-base font-semibold text-gray-900">Delete this listing?</h2>
            <p className="mt-1 text-sm text-gray-500">It will be removed from the marketplace (soft delete).</p>
