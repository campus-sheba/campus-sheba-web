...
import {
  deleteBookListingAction,
  fetchBookCategories,
  updateBookListingAction,
} from "@/services/book";
import { getAddressesAction } from "@/services/addresses";
import type { BookCategory, BookListing } from "@/types/book";
...
function categoryIdFromListing(listing: BookListing): string {
...
function addressIdFromListing(listing: BookListing): string {
...
function photosToMeta(listing: BookListing): UploadedMediaMeta[] {
...
export default function MyBookEditPage({ listing }: Props) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    const res = await updateBookListingAction(listingId, {
      title: title.trim(),
  ...
  async function onConfirmDelete() {
    setDeleting(true);
    setError(null);
    const res = await deleteBookListingAction(listingId);
    setDeleting(false);
    if (!res.success) {
      setError(res.message);
      return;
        ...
        <div className="mt-2 flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Update details or remove this listing.
            </p>
          </div>
          {listing.status ? (
      ...
      {deleteOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-xl border border-gray-200 bg-white p-5 shadow-lg">
            ...
            <p className="mt-1 text-sm text-gray-500">
              It will be removed from the marketplace (soft delete).
            </p>
