...
import { fetchBookCategories, fetchCreatorOwnBook } from "@/services/book";
...
export default function MyBookPage() {
  ...
  const fetchPage = async (nextPage: number, append: boolean) => {
    ...
    setError(null);
    try {
      const res = await fetchCreatorOwnBook(buildApiParams(nextPage));
      setTotal(res.total);
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Your campus book listings.</p>
        </div>
      ...
      {loading ? (
        ...
        <p className="rounded-xl border border-dashed border-gray-200 bg-white px-4 py-12 text-center text-sm text-gray-500">
          No listings yet. When you post items, they will appear here.
        </p>
      ) : (
