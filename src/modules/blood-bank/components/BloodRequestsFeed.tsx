...
export default function BloodRequestsFeed() {
  ...
  const { items, total, isLoading, error, hasMore, loadMore } = useBloodRequestsList({
    guestMode,
    universityId,
    pageSize: 12,
    ...
    <SectionWrapper spacing="none" background="transparent" className="my-0">
      <ContentWrapper maxWidth="max-w-7xl mx-auto" padding="md" className="pb-16 pt-2">
        ...
        <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 className="text-xl font-bold text-gray-900">{tt("bloodRequestsFeed.title", "Blood requests")}</h2>
            <p className="mt-1 text-sm text-gray-500">
              {tt("bloodRequestsFeed.subtitle", "Active and past emergency requests on your campus.")}
            </p>
          </div>
            ...
            {hasMore ? (
              <div className="mt-8 flex justify-center">
                <Button type="button" variant="outline" onClick={loadMore} disabled={isLoading}>
                  {isLoading ? tt("bloodRequestsFeed.loading", "Loading…") : tt("bloodRequestsFeed.loadMore", "Load more")}
                </Button>
              </div>
            ) : null}
