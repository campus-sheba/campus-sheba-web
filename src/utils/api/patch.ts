...
export async function patchPublic<T = unknown>(
  url: string,
  body: ApiBody = {},
  options: ApiRequestOptions = {},
): Promise<T> {
  ...
  return patch<T>(url, body, { ...options, headers });
}

export async function patchPrivate<T = unknown>(
  url: string,
  body: ApiBody = {},
  options: ApiRequestOptions = {},
): Promise<T> {
  const { privateRequest } = await import('./privateRequest');
  return privateRequest<T>('PATCH', url, body, options);
}

export async function patchThirdParty<T = unknown>(
  url: string,
  token: string,
  body: ApiBody = {},
  options: ApiRequestOptions = {},
